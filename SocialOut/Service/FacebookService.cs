...
namespace SocialOut.Service
{
    public class FacebookService : IFacebookService
    {
                    ...
                    try
                    {
                        ...
                        if (result.IsSuccessStatusCode)
                        {
                            ...
                            if (res.commentId != null)
                            {
                                ...
                                var resultDetailComment = await client.GetAsync(urlDetailComment);
                ...
                if (token != null)
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        try
                        {
                            ...
                            if (result.IsSuccessStatusCode)
                            {
                                Log.Information("Get fb information success");
                                var model = result.Content.ReadAsStringAsync().Result;
        ...
        public async Task<SendMessageResponseData> SendAttachment(IFormFile filedata, string senderId, string recipient, string message, string type)
        {
            try
            {
                Dictionary<string, string> FacebookPages = await _cacheService.GetFbInfo();
                string token;
                FacebookPages.TryGetValue(senderId, out token);
                if (token != null)
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        ...
                        byte[] fileBytes = new byte[filedata.Length + 1];
                        ...
                        string mimeType = GetMimeType(System.IO.Path.GetExtension(filedata.FileName));
                        
                        bytes.Headers.ContentType =
                                MediaTypeHeaderValue.Parse(mimeType);


                        content.Add(bytes, "file",filedata.FileName);
                        content.Add(new StringContent(JsonConvert.SerializeObject(new Recipient
                        {
                            id = recipient
                        })), "recipient");
                        ...
                        {
                            var model = result.Content.ReadAsStringAsync().Result;
                            SendMessageResponseData sendMessageResponse = JsonConvert.DeserializeObject<SendMessageResponseData>(model);
                ...
                {
                    Log.Error("--Reply message attachment missing token-- : \rPageId : " + senderId);
                }
                return null;
            }
            catch (Exception ex)
            {
                Log.Error("--Reply attachment fail--");
                return null;
            }
        }
